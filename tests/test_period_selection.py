...
def test_sales_outside_the_requested_period_are_refused():
    ...
    with pytest.raises(DataError, match='ни одной продажи'):
        build_snapshot([], {}, date(2026, 9, 21), date(2026, 9, 23))
...
def test_dated_olap_rows_carry_the_day_of_the_sale():
    ...
    assert parsed[0].cost == Decimal('80000')
    with pytest.raises(DataError, match='дату продажи'):
        director_rows_from_range([{**row, 'field0': {'value': 'вчера'}}])


def test_period_summary_splits_income_and_expense_and_carries_the_balance(tmp_path):
    store = FinanceStore(tmp_path / 'accountant.sqlite3')
    store.add_opening(date(2026, 9, 21), '1000000', 'Остаток на начало учёта')
    store.record_handover(date(2026, 9, 21), Decimal('500000'))
    store.record_handover(date(2026, 9, 22), Decimal('300000'))
    store.add_expense(date(2026, 9, 22), 'proc_bread', 'Хлеб на смену', '120000',
                      cashier_amount=Decimal('300000'))
    store.add_income(date(2026, 9, 22), 'income_other', 'Вернули долг', '80000')

    result = store.period_summary(date(2026, 9, 21), date(2026, 9, 22))

    assert result['period_days'] == 2
    assert [row['day'] for row in result['days']] == ['2026-09-21', '2026-09-22']
    assert [item['label'] for item in result['expense']] == ['Хлеб']
    assert result['expense_total'] == '120000'
    assert {item['label'] for item in result['income']} == {'Касса', 'Прочие поступления'}
    # Начальный остаток стоит отдельно: в приходе за период его быть не должно.
    assert result['income_total'] == '880000'
    assert [item['label'] for item in result['opening_entries']] == ['Остаток на начало учёта']
    assert result['days'][1]['received'] == '300000'
    # Остаток на конец периода совпадает с остатком последнего дня.
    assert result['closing_balance'] == result['days'][-1]['closing'] == '760000'
    with pytest.raises(LedgerError, match='позже'):
        store.period_summary(date(2026, 9, 22), date(2026, 9, 21))
...
def test_period_endpoints_refuse_a_broken_range_before_calling_iiko(tmp_path):
    ...
    with TestClient(app, base_url='http://127.0.0.1', client=('127.0.0.1', 50000)) as client:
        ...
        assert client.get('/api/accountant/period',
                          params={'start': future, 'end': future}).status_code == 422
