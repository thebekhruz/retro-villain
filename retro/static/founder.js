...
function renderPayments(data){
  ...
  const target=$('payment-chart');target.replaceChildren();if(!data.payment_series.length||!data.payment_summary.length){const empty=document.createElement('div');empty.className='empty-chart';empty.textContent='Нет данных для столбчатого графика.';target.append(empty);return}
  const directions=data.directions,payments=data.payment_summary.map(item=>item.name),barWidth=18,gap=8,groupGap=30;const groupWidth=directions.length*(barWidth+gap)+groupGap,width=Math.max(720,data.payment_series.length*groupWidth+60),height=255,base=120,chart=svg('svg',{viewBox:`0 0 ${width} ${height}`,'aria-hidden':'true'});chart.style.width=width+'px';
  const stacks=data.payment_series.flatMap(group=>directions.map(direction=>{const values=Object.values(group.directions[direction]||{}).map(Number);return {positive:values.filter(value=>value>0).reduce((sum,value)=>sum+value,0),negative:Math.abs(values.filter(value=>value<0).reduce((sum,value)=>sum+value,0))}}));const max=Math.max(1,...stacks.flatMap(stack=>[stack.positive,stack.negative]));chart.append(svg('line',{x1:20,y1:base,x2:width-10,y2:base,class:'chart-grid'}));
  data.payment_series.forEach((group,groupIndex)=>{directions.forEach((direction,directionIndex)=>{const values=group.directions[direction]||{};let positiveY=base,negativeY=base;payments.forEach((name,paymentIndex)=>{const amount=Number(values[name]||0),segment=Math.abs(amount)/max*80;if(segment){const y=amount>=0?(positiveY-=segment):negativeY;const rect=svg('rect',{x:38+groupIndex*groupWidth+directionIndex*(barWidth+gap),y,width:barWidth,height:segment,rx:2,fill:paymentColors[paymentIndex%paymentColors.length],class:'payment-bar'});if(amount<0)negativeY+=segment;const title=svg('title');title.textContent=`${shortDate(group.start)} · ${directionMeta[direction].label} · ${name}: ${money.format(amount)} сум`;rect.append(title);chart.append(rect)}});const marker=svg('text',{x:47+groupIndex*groupWidth+directionIndex*(barWidth+gap),y:base+98,'text-anchor':'middle',class:'chart-axis'});marker.textContent=directionMeta[direction].label[0];chart.append(marker)});const label=svg('text',{x:38+groupIndex*groupWidth+(directions.length*(barWidth+gap)-gap)/2,y:base+119,'text-anchor':'middle',class:'chart-axis'});label.textContent=shortDate(group.start)+(group.incomplete?' *':'');chart.append(label)});target.append(chart)
}
